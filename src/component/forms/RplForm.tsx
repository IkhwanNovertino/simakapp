...
const RplForm = ({ setOpen, type, data, relatedData }: FormProps) => {
  ...
  const [state, formAction] = useActionState(createRplTranscript, { success: false, error: false, message: "" });
        ...
        {data && (
          <div className="hidden">
            ...
            <InputField
              label="PeriodId"
              name="periodId"
              defaultValue={data?.periodId || period.id}
              register={register}
              error={errors?.studentId}
            />
          </div>
