...
const StudentTakingThesisPdf = ({ data, img }: GeneratePdfProps) => {
  const imgLogo = `${process.env.NEXT_PUBLIC_SITE_URL}/logo.png`

  return (
    <Document>
      {data?.dataStudentsTakingThesis?.map((items: any) => (
        <Page size={"A4"} style={styles.page} key={items?.major?.id}>
          <View>
            ...
            <View style={styles.body}>
              <Text style={[styles.textHeading1, { marginTop: "14px" }]}>DAFTAR MAHASISWA YANG MENGAMBIL TA PROGRAM STUDI {items?.major?.name}</Text>
...
export default StudentTakingThesisPdf;