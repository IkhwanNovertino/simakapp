...
const StudentRegularSorePdf = ({ data }: GeneratePdfProps) => {
  // const imgLogo = `${process.env.NEXT_PUBLIC_SITE_URL}/logo.png`

  return (
    <Document>
      {data?.dataStudents?.map((items: any) => (
        <Page size={"A4"} style={styles.page} key={items?.campusType}>
          <View>
            ...
            <View style={styles.body}>
              <Text style={[styles.textHeading1, { marginTop: "14px" }]}>DAFTAR MAHASISWA Regular {items?.campusType}</Text>
              ...
              <View style={[styles.table, { marginTop: "18px" }]}>
                <View style={styles.tableRow}>
                  ...
                  <View style={[styles.tableCol, { width: "14%" }]}>
                    <Text style={styles.tableCellTh}>Perkuliahan</Text>
                  </View>
                </View>
                {items?.students.length > 0 ? items?.students.map((student: any, index: number) => (
                  <View key={student.id || index} style={styles.tableRow} wrap={false}>
                    ...
                    <View style={[styles.tableCol, { width: "14%" }]}>
                      <Text style={[styles.tableCellTd, { margin: 0, textAlign: "left" }]}>{student?.campusType}</Text>
                    </View>
                  </View>
                )) : (
...
export default StudentRegularSorePdf;
