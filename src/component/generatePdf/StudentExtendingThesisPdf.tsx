...
const StudentExtendingThesisPdf = ({ data }: GeneratePdfProps) => {
  const imgLogo = `${process.env.NEXT_PUBLIC_SITE_URL}/logo.png`

  return (
    <Document>
      {data?.dataStudentsExtendingThesis?.map((items: any) => (
        <Page size={"A4"} style={styles.page} key={items?.major?.id}>
          <View>
            ...
            <View style={styles.body}>
              <Text style={[styles.textHeading1, { marginTop: "14px" }]}>DAFTAR MAHASISWA PROGRAM PERPANJANGAN TA</Text>
...
export default StudentExtendingThesisPdf;
